...
export default function Show({ curso }) {
    ...
    const [alumnos, setAlumnos] = useState(curso.inscripciones || []);

    const updateAlumno = (id, field, value) => {
        setAlumnos((current) =>
            current.map((ins) => (ins.id === id ? { ...ins, [field]: value } : ins)),
        );
    };

    const guardarAcreditacion = () => {
        router.post(
            `/admin/cursos/${curso.id}/acreditacion`,
            { alumnos },
            {
                preserveScroll: true,
                onSuccess: () => alert('Calificaciones guardadas con exito.'),
            },
        );
    };

    const darDeBaja = (inscripcionId) => {
        const motivo = prompt('Motivo de baja administrativa:') || 'Sin motivo capturado';
        router.post(
            `/admin/cursos/${curso.id}/alumnos/${inscripcionId}/baja`,
            { motivo },
            { preserveScroll: true },
        );
    };
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                {curso.unidad_capacitacion} | {curso.dias_semana} ({curso.hora_inicio.substring(0, 5)} a {curso.hora_fin.substring(0, 5)})
                            </p>
                        </div>
                        ...
                        {activeTab === 'alumnos' && (
                            <div className="animate-[fadeIn_0.3s_ease]">
                                <div className="dash-panel-header flex justify-between items-center bg-gray-50/50 border-b border-[rgba(107,18,48,.09)]">
                                    ...
                                    <a href={`/admin/cursos/${curso.id}/lista`} target="_blank" className="btn-mas-info !text-[#6B1230] !border-[#6B1230] hover:!bg-[#6B1230] hover:!text-white">
                                        Imprimir Lista
                                    </a>
                                </div>
                                            ...
                                            {alumnos.map((ins, idx) => (
                                                <tr key={ins.id}>
                                                    ...
                                                    <td className="text-center">
                                                        <button onClick={() => darDeBaja(ins.id)} className="text-xs font-bold text-gray-500 hover:text-red-600 transition">
                                                            Dar de baja
                                                        </button>
                                                    </td>
                                                </tr>
                                            ...
                                            {alumnos.length === 0 && (
                                                <tr>
                                                    <td colSpan="5" className="text-center py-12 text-gray-500">
                                                        Aun no hay alumnos validados en este grupo.
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
                            ...
                            <div className="animate-[fadeIn_0.3s_ease]">
                                <div className="dash-panel-header flex justify-between items-center bg-gray-50/50 border-b border-[rgba(107,18,48,.09)]">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">Cierre oficial de grupo. Captura la calificacion y el resultado final.</p>
                                    </div>
                                    <button onClick={guardarAcreditacion} className="btn-mas-info !bg-[#6B1230] !text-white hover:!bg-[#4A0B20]">
                                ...
                                <div className="cursos-table-wrap !p-0">
                                    <table className="cursos-table !mt-0 w-full">
                                        <thead>
                                            <tr>
                                                <th className="w-12 text-center">No.</th>
                                                <th>Numero de Control / Alumno</th>
                                                <th className="w-32 text-center">Calificacion (0-10)</th>
                                                ...
                                                <tr key={ins.id} className={ins.resultado === 'baja' ? 'opacity-50 bg-gray-50' : ''}>
                                                    ...
                                                    <td className="text-center p-2">
                                                        <input
                                                            ...
                                                            max="10"
                                                            step="0.1"
                                                            value={ins.calificacion || ''}
                                                            onChange={(e) => updateAlumno(ins.id, 'calificacion', e.target.value)}
                                                    ...
                                                    <td className="text-center p-2">
                                                        <select
                                                            value={ins.resultado || 'pendiente'}
                                                            onChange={(e) => updateAlumno(ins.id, 'resultado', e.target.value)}
                                                            className="w-full rounded border-gray-300 shadow-sm focus:border-[#6B1230] focus:ring-[#6B1230] text-sm py-1.5 font-semibold text-gray-600"
                                                        >
                                                    ...
                                                    <td className="text-center">
                                                        {ins.resultado === 'aprobado' ? (
                                                            <a
                                                                href={`/admin/cursos/${curso.id}/alumnos/${ins.id}/constancia`}
                                                                target="_blank"
                                                                className="text-xs font-bold text-blue-700 hover:text-blue-900 bg-blue-50 px-3 py-1.5 rounded border border-blue-200"
                                                            >
                                                                Generar
                                                            </a>
                                                        ) : (
